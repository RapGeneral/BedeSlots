...
namespace BedeSlots.Areas.Admin.Controllers
{
    ...
    public class UsersController : Controller
    {
        ...
        public async Task<IActionResult> LockUser(string userId, int durationInDays)
        {
            if (durationInDays < 1)
            {
                return PartialView("_StatusMessage", "Error: How do you expect to lock the user back in time?");
            }
            if (durationInDays > 36000)
            {
                return PartialView("_StatusMessage", "Error: Do you even know that 36000 days are nearly 100 years?");
            }
            ...
            {
                return PartialView("_StatusMessage", "Error: Could enable the lockout on the user!");
            }
            ...
            {
                return PartialView("_StatusMessage", "Error: Could not add time to user's lockout!");
            }
            return PartialView("_StatusMessage", $"The user has been successfully locked for {durationInDays} days!");
        }
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> UnlockUser(string userId)
        {
            ...
            return PartialView("_StatusMessage", "The user has been successfully unlocked!");
        }
        ...
        public async Task<IActionResult> DemoteUser(string userId)
        {
            ...
            if (!result.Succeeded)
            {
                return PartialView("_StatusMessage", "Error: Could not demote user!");
            }
            return PartialView("_StatusMessage", "The user has been successfully demoted!");
        }
    }
}